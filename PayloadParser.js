function parseUplink(device, payload)
{
    ...
    // Store fire alarm
    if(decoded.fireAlarm != null){
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateGenericSensorStatus(decoded.fireAlarm);
    };

    // Store high temp alarm
    if(decoded.highTempAlarm != null){
        var sensor3 = device.endpoints.byAddress("3");

        if (sensor3 != null)
            sensor3.updateGenericSensorStatus(decoded.highTempAlarm);
    };
...
function Decoder(bytes, fport) {
	...
	if (fport === 6){ // then its ReportDataCmd
		if (bytes[2] === 0x00) { // version report
			...
			decoded.devicetype = "RA02A";
			decoded.battery = (bytes[3] & 0b01111111) / 10;
            decoded.lowBattery = (bytes[3] & 0b10000000) != 0;
            decoded.fireAlarm = bytes[4];
            decoded.highTempAlarm = bytes[5];
            decoded.temperature = ((bytes[6] << 24 >> 16) + bytes[7]) / 10;
            return decoded;
        }
